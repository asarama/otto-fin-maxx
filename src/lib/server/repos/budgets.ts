...
export async function listBudgets(conn: DuckDBConnection): Promise<Budget[]> {
	...
	return reader
		.getRowObjects()
		.map((r) => ({ id: String(r.id), name: String(r.name), owner_id: String(r.owner_id) }));
}
...
export async function updateBudgetCategoryLimit(
	...
	await conn.run('UPDATE budget_categories SET monthly_limit_cents = ? WHERE id = ?', [
		monthlyLimitCents,
		id,
	]);
	await conn.run('UPDATE budget_category_months SET amount_cents = ? WHERE id = ?', [
		monthlyLimitCents,
		current.id,
	]);
}
...
export async function ensureBudgetCategoryMonth(
	...
	const limit = await conn.runAndReadAll(
		'SELECT monthly_limit_cents FROM budget_categories WHERE id = ?',
		[budgetCategoryId]
	);
	const amountCents =
		limit.getRowObjects().length > 0 ? Number(limit.getRowObjects()[0].monthly_limit_cents) : 0;
	const id = randomUUID();
	await conn.run(
...
export async function listBudgetCategoryMonths(
	conn: DuckDBConnection,
	month: string
): Promise<BudgetCategoryMonth[]> {
	const reader = await conn.runAndReadAll(
