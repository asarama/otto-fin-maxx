...
async function seedTx(
	conn: Awaited<ReturnType<typeof createTestDb>>,
	...
	await conn.run(
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		[
			overrides.id ?? 'tx1',
			account.id,
			overrides.externalId ?? 'e1',
			overrides.postedDate ?? '2026-07-01',
			overrides.description ?? 'AMZN MKTP US',
			overrides.rawVendorName ?? 'AMZN MKTP US',
			overrides.amountCents ?? -500,
			vendor.id,
			overrides.status ?? 'unreviewed',
			'2026-07-01',
		]
	);
