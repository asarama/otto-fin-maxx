	...
	for (const row of rules.getRowObjects()) {
		...
		if (!byId.has(id)) {
			byId.set(id, {
				...
				descriptionMatcher:
					row.description_matcher === null ? null : String(row.description_matcher),
	...
	await conn.run(
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?, true)`,
		[
			id,
			input.name,
			input.descriptionMatcher ?? null,
			input.amountOperator ?? 'any',
...
export async function updateRule(
	...
	if (patch.descriptionMatcher !== undefined) {
		sets.push('description_matcher = ?');
		params.push(patch.descriptionMatcher);
	}
	if (patch.amountOperator !== undefined) {
		sets.push('amount_operator = ?');
		params.push(patch.amountOperator);
	}
	...
	if (patch.budgetCategoryId !== undefined) {
		sets.push('budget_category_id = ?');
		params.push(patch.budgetCategoryId);
	}
	...
	const neighbor =
		direction === 'up'
			? await conn.runAndReadAll(
					'SELECT id, priority FROM rules WHERE priority < ? ORDER BY priority DESC LIMIT 1',
					[from]
				)
			: await conn.runAndReadAll(
					'SELECT id, priority FROM rules WHERE priority > ? ORDER BY priority ASC LIMIT 1',
					[from]
				);
